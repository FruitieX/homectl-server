...
mod tests {
    ...
    use crate::types::{group::GroupId, scene::SceneId, websockets::WebSocketResponse};

    #[test]
    fn test_is_rule_triggered() {
        let fixtures_file =
            std::fs::read_to_string("./src/core/state-downstairs-on-upstairs-off.json").unwrap();
...
devices = [
  ...
  { integration_id = "tuya", name = "Living room downlight 4" },
        ...
        let rule = Rule::Group(GroupRule {
            group_id: GroupId::from_str("living_room").unwrap(),
            power: None,
            scene: Some(SceneId::from_str("normal").unwrap()),
        });
